    ...
    public class QLsanPhamChiTietServices : IQLsanPhamChiTietServices
    {
        ...
        public List<ViewHienThi1> getViewSanPhamCT()
        {
            _lstView = (from a in GetSanPhamCTTFromDB()
                        ...
                        join f in _QLchatLieuServices.GetchatLieuFromDB() on a.IDChatlieu equals f.IDChatlieu
                        select new ViewHienThi1 { SanPhamChiTiets = a, mauSacs = b, hangSXs = c, sizes = d, sanPhams = e ,chatLieus = f}).ToList();
            return _lstView;
        }
    }
}
