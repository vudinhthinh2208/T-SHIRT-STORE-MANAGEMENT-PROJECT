...
namespace _3.GUI.View.BanHang
{
    public partial class FrmGioHang : Form
    {
        ...
        public void LoadChatLieu()
        {
            ...
            cmb_chatlieu.Items.Add("Tất cả");
            var cl = _qLchatLieuServices.GetchatLieuFromDB().Where(x => x.trangThai == true);
            foreach (var item in cl)
            {
                cmb_chatlieu.Items.Add(item.ChatLieu.ToString());
            }
        ...
        public void LoadSanPham()
        {
            ...
            {
                dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP, item.SanPhamChiTiets.TenSP, item.mauSacs.tenMau, item.sizes.SiZe, item.SanPhamChiTiets.Giaban, item.hangSXs.tenHangSX,item.chatLieus.ChatLieu, item.SanPhamChiTiets.Soluong);
            }
        }
        ...
        public void botgiohang(int pID)
        {
            var p = _qlSanPhamCT.getViewSanPhamCT().FirstOrDefault(x => x.SanPhamChiTiets.IDSanPhamChiTiet == pID);
            var data = _lstViewHoaDon.FirstOrDefault(x => x.ID == p.SanPhamChiTiets.IDSanPhamChiTiet);
            if (p.SanPhamChiTiets.Soluong <= 0)
            ...
            else
            {
                ...
                if(data.soLuong <= 1)
                {
                    MessageBox.Show("Số lượng sản phẩm này trong giỏ hàng cần >= 1");
                }
                else
                {
                    data.soLuong--;
        ...
        private void dtgv_gioHang_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 /*|| rowIndex == -1*/) /*return*/;
            {
                DataGridViewRow r = dtgv_gioHang.Rows[e.RowIndex];
        ...
        private void btn_TaoHoaDon_Click(object sender, EventArgs e)
        {
            if (_lstViewHoaDon.Any())
            {
                ...
                int IDNV = _qLnhanVienServices.GetNhanVienFromDB().FirstOrDefault(x => x.nhanViens.email == BUS.Properties.Settings.Default.tk).nhanViens.IDNhanVien;
                if (KH != null)
                {
                    ...
                    //lbl_tongTien.Text = "";
                    MessageBox.Show($"Tạo hóa đơn thành công. Hóa Đơn: {HD.IDHoaDon}");
                    loadHDcho();
                    _lstViewHoaDon = new List<ViewHoaDonCT>();
                }
                else
                {
                    MessageBox.Show("Điền thông tin của khách hàng");
                }
            }
            else
            {
            ...
            if (oID != -1)
            {
                if (_lstViewHoaDon.Any())
                {
                    ...
                    if (KH != null)
                    {
                        ...
                        lbl_giamGia.Text = "";
                        tbt_maHD.Text= "";
                        MessageBox.Show($"Cập nhật hóa đơn thành công. Hóa đơn: {oID}");
                        oID = -1;
                        loadHDcho();
                        dtgv_gioHang.Rows.Clear();
                    }
                    ...
                    {
                        MessageBox.Show("Nhập thông tin của khách hàng");
                    }
                }
                else
                {
        ...
        private void btn_thanhToan_Click(object sender, EventArgs e)
        {
           hoaDon HD = _qlhoaDon.GetHoaDonFromDB().FirstOrDefault(p => Convert.ToString(p.IDHoaDon) == tbt_maHD.Text && p.trangThai == false);
            ...
            else
            {
                ...
                int x;
                if (tbt_giamGia.Text == "" || float.Parse(tbt_giamGia.Text) < 0 || float.Parse(lbl_TienThua.Text) < 0 || tbt_tienKhachDua.Text == "" || 
                    (!float.TryParse(tbt_giamGia.Text, out float z) && tbt_giamGia.Text != "") || !float.TryParse(tbt_tienKhachDua.Text, out float y) || 
                    float.Parse(tbt_giamGia.Text) > float.Parse(lbl_tongTien.Text) )
                {
                    MessageBox.Show("Vui lòng nhập đúng số tiền"+ lbl_Diem.Text);
                }
                else if (float.Parse(tbt_giamGia.Text) > float.Parse(lbl_Diem.Text))
                {
                    MessageBox.Show("Điểm sử dụng <= Điểm");
                }
            ...
            if (cbb_NSX.Text == "Tất cả")
            {
                ...
                foreach (var item in _qlSanPhamCT.getViewSanPhamCT())
                {
                    dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP, item.SanPhamChiTiets.TenSP,
                        item.mauSacs.tenMau, item.sizes.SiZe, item.SanPhamChiTiets.Giaban, item.hangSXs.tenHangSX,item.chatLieus.ChatLieu, item.SanPhamChiTiets.Soluong);
                }
            }
            else
            {
                ...
                foreach (var item in timNSX)
                {
                    dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP, item.SanPhamChiTiets.TenSP, item.mauSacs.tenMau, item.sizes.SiZe,
                        item.SanPhamChiTiets.Giaban, item.hangSXs.tenHangSX,item.chatLieus.ChatLieu, item.SanPhamChiTiets.Soluong);
                }
            }
        }
            ...
            if (cbb_size.Text == "Tất cả")
            {
                ...
                foreach (var item in _qlSanPhamCT.getViewSanPhamCT())
                {
                    dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP, item.SanPhamChiTiets.TenSP, item.mauSacs.tenMau,
                        item.sizes.SiZe, item.SanPhamChiTiets.Giaban, item.hangSXs.tenHangSX,item.chatLieus.ChatLieu, item.SanPhamChiTiets.Soluong);
                }
            }
            else
            {
                ...
                foreach (var item in timsize)
                {
                    dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP,
                        item.SanPhamChiTiets.TenSP, item.mauSacs.tenMau, item.sizes.SiZe, item.SanPhamChiTiets.Giaban, item.hangSXs.tenHangSX,item.chatLieus.ChatLieu, item.SanPhamChiTiets.Soluong);
                }
            }
        }
        ...
        private void pdhd_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            //
            e.Graphics.DrawString("Cửa Hàng Áo Phông TShirt", new System.Drawing.Font("Times New Roman", 15, FontStyle.Bold), Brushes.Black, new System.Drawing.Point(100, 20));
        ...
        private void cmb_chatlieu_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var timchatlieu = _qlSanPhamCT.getViewSanPhamCT().Where(p => p.chatLieus.ChatLieu == cmb_chatlieu.Text);
                dtgv_sanPham.Rows.Clear();
                foreach (var item in timchatlieu)
                {
                    dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP,
                        item.SanPhamChiTiets.TenSP, item.mauSacs.tenMau, item.sizes.SiZe, item.SanPhamChiTiets.Giaban, item.hangSXs.tenHangSX, item.chatLieus.ChatLieu, item.SanPhamChiTiets.Soluong);
                }
