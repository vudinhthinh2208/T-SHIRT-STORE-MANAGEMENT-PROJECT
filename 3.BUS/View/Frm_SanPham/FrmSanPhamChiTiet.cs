...
namespace _3.GUI.View.FromSanPham
{
    public partial class FrmSanPhamChiTiet : Form
    {
        ...
        public void LoadChatLieu()
        {
            ...
            {
                cbb_chatlieu.Items.Add(item.ChatLieu.ToString());
            }
        ...
        public void loadDuLieu()
        {
            ...
            {
                dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet,item.SanPhamChiTiets.maSP, item.SanPhamChiTiets.TenSP, item.SanPhamChiTiets.Gianhap, item.SanPhamChiTiets.Giaban, item.SanPhamChiTiets.Soluong,item.mauSacs.tenMau, item.sizes.SiZe,
                    item.hangSXs.tenHangSX, item.sanPhams.tenLoaiHang,
                    item.chatLieus.ChatLieu,item.SanPhamChiTiets.Trangthai == true ? "Còn hàng" : "Hết hàng");
            }
        }
        ...
        private bool checkValidate()
        {
            if ( tbt_ten.Text.Trim() == "" || tbt_giaNhap.Text.Trim() == "" 
                ...
                || cb_size.Text.Trim() == ""|| cb_NSX.Text.Trim() =="" || cbb_loaiHang.Text.Trim() == "" || cbb_chatlieu.Text.Trim() =="") return false;
            return true;
        }
        ...
        {
            if (e.RowIndex >= 0 && dtgv_sanPham.Rows[e.RowIndex].Cells[1].Value != null)
            {
                ...
                cbb_chatlieu.Text = row.Cells[10].Value.ToString();
                rb_con.Checked = row.Cells[11].Value.ToString() == "Còn hàng" ? true : false;
                rb_het.Checked = row.Cells[11].Value.ToString() == "Hết hàng" ? true : false;
        ...
        private void btn_capNhat_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else if(Convert.ToInt32(tbt_soLuong.Text) < 0 || Convert.ToInt32(tbt_soLuong.Text) > 9999)
                {
                    MessageBox.Show("Số lượng sản phẩm >=0 và <= 9999");
                }
                else
                {
                    ...
                    update.IDsanPham = _qLsanPhamServices.GetsanPhamFromDB()[cbb_loaiHang.SelectedIndex].IDsanPham;
                    update.IDChatlieu = _qLchatLieuServices.GetchatLieuFromDB()[cbb_loaiHang.SelectedIndex].IDChatlieu;
        ...
        private void btn_them_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                sanPhamChiTiet sp = new sanPhamChiTiet()
                {
                    ...
                    IDChatlieu = _qLchatLieuServices.GetchatLieuFromDB()[cbb_chatlieu.SelectedIndex].IDChatlieu,
        ...
        private void tbt_timKiem_TextChanged_1(object sender, EventArgs e)
        {
            ...
            || x.sizes.SiZe.ToLower().Contains(tbt_timKiem.Text) || x.sanPhams.tenLoaiHang.ToLower().Contains(tbt_timKiem.Text) || x.chatLieus.ChatLieu.ToLower().Contains(tbt_timKiem.Text)
            || Convert.ToString(x.SanPhamChiTiets.IDsanPham).Contains(tbt_timKiem.Text)))
            {
                dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP, item.SanPhamChiTiets.TenSP, item.SanPhamChiTiets.Gianhap, item.SanPhamChiTiets.Giaban, item.SanPhamChiTiets.Soluong, item.mauSacs.tenMau, item.sizes.SiZe, item.hangSXs.tenHangSX, item.sanPhams.tenLoaiHang,item.chatLieus.ChatLieu,
