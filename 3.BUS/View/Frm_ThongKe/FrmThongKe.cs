...
namespace _3.GUI.View
{
    public partial class FrmThongKe : Form
    {
        ...
        private IQLnhanVienServices _worker;
        ...
        public FrmThongKe()
        {
            ...
            _worker = new QLnhanVienServices();
        ...
        public int tinhvon(int id)
        {
            int tv = 0;
            var x = _orderdetail.GetHoaDonChiTietFromDB().Where(p => p.IDHoaDon == id);
            foreach (var item in x)
            {
                var sp = _product.GetSanPhamCTTFromDB().Where(p => p.IDSanPhamChiTiet == item.IDSanPham);
                foreach(var item2 in sp)
                {
                    int gsp = Convert.ToInt32(item2.Gianhap);
                    tv = tv + (gsp * item.Soluong);
                }
            }
            return tv;
        }
        public int tinhln(int dt,int gv)
        {
            int ln = dt - gv;
            return ln;
        }
        public void loadData()
        {
            ...
            dtgv_show.Columns[1].Name = "Giá trị hóa đơn";
            //dtgv_show.Columns[5].Name = "Tổng tiền trong hóa đơn";
            dtgv_show.Columns[2].Name = "Tiền vốn sản phẩm";
            dtgv_show.Columns[3].Name = "Lợi nhuận cơ bản";
            dtgv_show.Columns[4].Name = "Ngày bán";
            dtgv_show.Columns[5].Name = "SDT khách hàng";
            dtgv_show.Columns[6].Name = "Trạng thái";
            dtgv_show.Rows.Clear();         
            foreach (var i in _lstOrder)
            {
                dtgv_show.Rows.Add(i.IDHoaDon, i.tongTien + " VNĐ", tinhvon(i.IDHoaDon)+" VNĐ",tinhln((int)i.tongTien,tinhvon(i.IDHoaDon)) + " VNĐ", i.ngayBan,i.SDT_KH, i.trangThai == true ? "Đã thanh toán" : "Chưa thanh toán");
            }
            ...
            dtgv_show.AutoSizeColumnsMode=DataGridViewAutoSizeColumnsMode.DisplayedCells;
        }
